...
def _apply_runtime_config(
    ...
    roll_deg: float,
    ssaa_scale: int,
    celestial_scale_enabled: bool | None,
    dynamic_saturation: bool | None,
    saturation_angular_blend: float | None,
    brightness_angular_blend: float | None,
    trail_base_resolution_factor: float | None,
    trail_step_scale: float | None,
    orbit_radius_mode: str | None,
    orbit_radius_power: float | None,
    center_body: str | None,
    ...
    config.VIEW_PITCH_DEG = float(pitch_deg)
    config.VIEW_ROLL_DEG = float(roll_deg)
    config.SSAA_SCALE = max(1, int(ssaa_scale))
    if celestial_scale_enabled is not None:
        config.SHOW_CELESTIAL_SCALE = bool(celestial_scale_enabled)
    if dynamic_saturation is not None:
        config.TRAIL_DYNAMIC_SATURATION = bool(dynamic_saturation)
    if saturation_angular_blend is not None:
        config.TRAIL_SATURATION_ANGULAR_BLEND = float(saturation_angular_blend)
    if brightness_angular_blend is not None:
        config.TRAIL_BRIGHTNESS_ANGULAR_BLEND = float(brightness_angular_blend)
    if trail_base_resolution_factor is not None:
        config.set_trail_base_resolution_factor(float(trail_base_resolution_factor))
    if trail_step_scale is not None:
        config.set_trail_step_scale(float(trail_step_scale))
    if orbit_radius_mode is not None:
        config.ORBIT_RADIUS_MODE = str(orbit_radius_mode).strip().lower()
    if orbit_radius_power is not None:
        config.ORBIT_RADIUS_POWER = float(orbit_radius_power)
    if selection_expression is not None:
        config.set_render_selection_expression(selection_expression)
    else:
        config.set_render_selection_expression(None)
    if center_body is not None:
        config.set_observer_center_body(center_body)

    center_key = str(config.OBSERVER_CENTER_BODY).strip().lower()
    if selection_expression is not None and not config.selection_expression_includes_body(selection_expression, center_key):
        raise ValueError(
            f"Center body '{center_key}' is not included by selection expression '{selection_expression}'"
        )
...
def run_once(
    ...
    _apply_runtime_config(
        ...
        celestial_scale_enabled=celestial_scale_enabled,
        dynamic_saturation=dynamic_saturation,
        saturation_angular_blend=saturation_angular_blend,
        brightness_angular_blend=brightness_angular_blend,
        trail_base_resolution_factor=trail_base_resolution_factor,
        trail_step_scale=trail_step_scale,
        orbit_radius_mode=orbit_radius_mode,
        orbit_radius_power=orbit_radius_power,
    ...
    if (
        ...
        and not ephemeris.is_trail_update_due(now_utc)
        and not render.is_overlay_update_due()
    ):
        return
